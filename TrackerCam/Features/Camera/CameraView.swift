...
struct CameraView: View {
    ...
    private func sourceNormalizedPoint(from location: CGPoint,
        ...
        guard let p = PreviewGeometry.normalizedTexturePoint(from: location,
                                                             viewSize: viewSize,
                                                             outputSize: outputSize,
                                                             aspectFill: aspectFill) else { return nil }
        let crop = viewModel.cropInSourceRect ?? CGRect(x: 0, y: 0, width: 1, height: 1)
        return CGPoint(x: crop.minX + p.x * crop.width,
                       y: crop.minY + p.y * crop.height)
    }
